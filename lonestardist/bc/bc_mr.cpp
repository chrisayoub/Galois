...
#ifdef __GALOIS_HET_CUDA__
	if (personality == GPU_CUDA) {
		InitializeIteration_allNodes_cuda(cuda_ctx, nodesToConsider, numSourcesPerRound);
	} else if (personality == CPU)
#endif
  galois::do_all(
      galois::iterate(allNodes.begin(), allNodes.end()),
...
void BackProp(Graph& graph, const uint32_t lastRoundNumber) {
  ...
  while (currentRound <= lastRoundNumber) {
    ...
    galois::do_all(
        ...
        galois::loopname(
            syncSubstrate->get_run_identifier("BackProp").c_str()),
        galois::steal(),
        galois::no_stats());
    ...
    while (offset < nodeBoundary && totalSourcesFound < totalNumSources) {
      ...
      if (galois::runtime::getSystemNetworkInterface().ID == 0) {
        ...
        galois::runtime::reportStat_Single(REGION_NAME,
          ...
          lastRoundNumber + 1);
        galois::runtime::reportStat_Single(REGION_NAME,
          syncSubstrate->get_run_identifier("TotalRounds"),
